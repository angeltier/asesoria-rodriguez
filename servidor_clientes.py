...
def cargar_datos():
    ...
    for path, nombre in [(CLIENTES_FILE,"clientes"),(TRAMITES_FILE,"tramites")]:
        if os.path.exists(path):
            try:
                with open(path,"r",encoding="utf-8") as f: data=json.load(f)
                if nombre=="clientes": clientes_db=data
                else: tramites_db=data
...
app = FastAPI(title="Gestion de Clientes", lifespan=lifespan)
...
async def listar_clientes(q:Optional[str]=None, _=Depends(verificar_auth)):
    ...
    if q:
        ...
        cs=[c for c in cs if ql in (c.get("nombre","")+" "+c.get("ap_paterno","")+" "+c.get("ap_materno","")).lower()
            or ql in c.get("telefono","").lower() or ql in c.get("email","").lower()
...
async def actualizar_cliente(cid:str, data:dict, _=Depends(verificar_auth)):
    ...
    c.update(data); c["fecha_actualizacion"]=now_iso(); guardar_clientes(); return c
...
async def actualizar_tramite(tid:str, data:dict, _=Depends(verificar_auth)):
    ...
    t.update(data); t["fecha_actualizacion"]=now_iso(); guardar_tramites(); return t
